...
resource "azurerm_site_recovery_replicated_vm" "windows_svr" {
  ...
  target_resource_group_id                = data.terraform_remote_state.vm.outputs.windows_svr_rg[count.index][data.terraform_remote_state.vm.outputs.windows_svr[count.index].resource_group_name]
  target_recovery_fabric_id               = azurerm_site_recovery_fabric.secondary.id
  target_recovery_protection_container_id = azurerm_site_recovery_protection_container.secondary.id
...
resource "azurerm_site_recovery_replicated_vm" "windows_wks" {
  ...
  target_resource_group_id                = data.terraform_remote_state.vm.outputs.windows_wks_rg[count.index][data.terraform_remote_state.vm.outputs.windows_wks[count.index].resource_group_name]
  target_recovery_fabric_id               = azurerm_site_recovery_fabric.secondary.id
  target_recovery_protection_container_id = azurerm_site_recovery_protection_container.secondary.id
