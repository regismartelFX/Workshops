...
module "Configure_backup_on_virtual_machines_with_a_given_tag_to_an_existing_recovery_services_vault_in_the_same_location_cc_DEMO_d" {
  ...
  subscription = {
    id   = data.azurerm_subscription.demo.id
    name = data.azurerm_subscription.demo.display_name
  }
  not_scopes        = []
  ...
  backupPolicyId    = data.azurerm_backup_policy_vm.demo_daily.id
  inclusionTagValue = ["${data.azurerm_backup_policy_vm.demo_daily.name}"]
  depends_on = [
    data.azurerm_subscription.demo,
    data.azurerm_backup_policy_vm.demo_daily
  ]
